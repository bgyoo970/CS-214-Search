...
char *createFileStr(char *argv) {
	...
	/* Open the file and check if the file is NULL */
	FILE *fp;
	char *textfilename;
	textfilename = argv;
	...
	/* store the textfile as a string into a char array called tkfilestr */
	int i;
	char c;
	char* tkFileStr = NULL;
	if (fp) {
		...
		tkFileStr = malloc(sizeof(char) * len + 1);
		tkFileStr[len] = '\0';
